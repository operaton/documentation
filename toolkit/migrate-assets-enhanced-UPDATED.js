...
function isAssetFile(filename) {
  ...
  // Exclude Docusaurus configuration files
...
function getOrphanedAssetPath(oldPath) {
  ...
  // Use 'examples' instead of 'orphaned' for BPMN/DMN reference files
  return path.join(CONFIG.staticPath, sidebarSection, 'examples', relativePath);
}
...
function printReport() {
  ...
  if (report.orphanedAssets.length > 0) {
    ...
    console.log('They have been moved to static/img/*/examples/ directories.');
    console.log('\nRecommendations:');
  ...
  if (report.errors.length > 0) {
    console.log(`\nâŒ Errors: ${report.errors.length}`);
