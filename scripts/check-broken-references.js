...
/**
 * Broken Asset Reference Checker (Updated for docs/assets)
 * 
 * Scans all markdown files and reports asset references that don't exist.
 * Works with both docs/assets/ (relative paths) and static/img/ (absolute paths).
 * 
 * Usage:
 *   node check-broken-references.js
 *   node check-broken-references.js --verbose
 *   node check-broken-references.js --report-only
 */
...
const CONFIG = {
  docsPath: './docs',
  staticPath: './static/img',  // For legacy /img/ references if any
  assetExtensions: [
...
function extractAssetReferences(content, filePath) {
  ...
  while ((match = mdRegex.exec(content)) !== null) {
    ...
    // Skip mailto links
    if (assetPath.startsWith('mailto:')) {
      continue;
    }
...
async function checkAssetExists(assetPath, mdFilePath) {
  // Absolute path from /img/ (legacy static/img references)
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          // Skip hidden, node_modules, and versioned_docs
  ...
  try {
    ...
    for (const asset of assets) {
      ...
      if (!check.exists) {
        ...
        if (CONFIG.verbose) {
          console.log(`❌ ${path.relative(CONFIG.docsPath, filePath)}:${asset.lineNumber}`);
          console.log(`   ${asset.path} → ${check.resolvedPath}`);
        }
      } else {
...
async function main() {
  ...
  for (const file of mdFiles) {
    ...
    if (!CONFIG.verbose && !CONFIG.reportOnly) {
  ...
  console.log(`External references: ${report.externalReferences}`);
  console.log(`Skipped (templates): ${report.skippedReferences}`);
