...
To configure a connector detected by Connect, the SPI
...
* Add the configurator's fully qualified class name to a file named `META-INF/services/org.operaton.connect.spi.ConnectorConfigurator`
* Ensure that the artifact containing the configurator is reachable from Connect's class loader
...
Connect uses the Java platform's service loader mechanism to look up provider
...
* Add the provider's fully qualified class name to a file named `META-INF/services/org.operaton.connect.spi.ConnectorProvider`
* Ensure that the artifact containing the provider is reachable from Connect's class loader
...
can be used to explicitly retrieve the connector by ID.
