...
The Cockpit processes dashboard is the entry point for process monitoring. It comes with a pre-installed plugin that lets you see deployed process definitions. Additional [plugins](../extend/plugins.md) can be added to the processes dashboard.
...
With this plugin, you can easily observe the state of a process definition. Green and red dots indicate running and [failed jobs][failed-jobs]. At this overview level, a red dot signifies that there is at least one process instance or subprocess instance with an unresolved incident. You can locate the problem by using the [process definition view][process-definition-view].
...
Both of these search properties support the 'like' operator, which lets you search for process definitions where the entered value is a substring of the property value.

Furthermore, you can copy a link to the current search query to your clipboard by clicking
the <button class="btn btn-xs"><i class="glyphicon glyphicon-link"></i></button> button and you can
save search queries to your local browser storage by clicking
the <button class="btn btn-xs"><i class="glyphicon glyphicon-floppy-disk"></i></button> button and
entering a name in the drop-down menu that appears. You can then retrieve the search query by
clicking the <button class="btn btn-xs"><i class="glyphicon glyphicon-floppy-disk"></i></button>
...
![Example img](/img/documentation/webapps/cockpit/bpmn/cockpit-search.png)Cockpit Search

At the top of the dashboard, you can search for process instances and incidents that match certain search criteria. To do so, click in the search box and select the parameters to search for. You can also begin typing to find the required parameter faster. Depending on the selected property, you have to specify the value of the property. Some properties also allow operators other than equals, e.g., 'like', which lets you search for process instances where the entered value is a substring of the property value. To search for process variables, you also have to enter the variable name you want to search for. You can search for variable values with the type `String`, `Number`, or `Boolean`. To search for a variable of type string, which has a numeric, boolean or null value, you have to wrap the value in single quotes (e.g., `'93288'` or `'NULL'`). You can combine multiple search pills to narrow down the search results.
...
You can perform batch operations on process instances matching search criteria by clicking the "Batch operation" button.
...
![Example img](/img/documentation/webapps/cockpit/bpmn/cockpit-export.png)CSV Export for Process Instances

To trigger the CSV export, search for process instances and customize which properties you want to export by adding or removing columns. Suppose you are only interested in selecting
specific process instances; you can select the process instances of interest on the left side or select the whole page by selecting
...
![Example img](/img/documentation/webapps/cockpit/bpmn/cockpit-export-modal-dialog.png)CSV Export for Process Instances: Modal Dialog
...
When you click **Export CSV**, the backend processes the requested data and creates a CSV file. This could take a while, depending on the number of process instances you want to export.
...
* The spreadsheet has a dedicated column for each variable property and displays its value in the row that matches the process instance.
* The spreadsheet displays:
  * A `null` value as `<<NULL>>`
  * An unsupported variable value type as `<<UNSUPPORTED TYPE>>`
  * A nonexistent variable as an empty cell.

#### Example

![Example img](/img/documentation/webapps/cockpit/bpmn/cockpit-export-result.png)CSV Export for Process Instances: Export Result
...
![Example img](/img/documentation/webapps/cockpit/bpmn/cockpit-in-operator.png)IN Operator
...
![Example img](/img/documentation/webapps/cockpit/bpmn/cockpit-in-operator-modal.png)IN Operator Modal
