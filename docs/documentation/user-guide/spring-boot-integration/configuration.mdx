...
data source with name `operatonBpmDataSource` that will be automatically wired with Operaton instead.
...
@Bean(name="operatonBpmDataSource")
...
transaction manager with the name `operatonBpmTransactionManager` that will be wired with the data
source used for Operaton(either `@Primary` or `operatonBpmDataSource`):
...
@Bean(name="operatonBpmTransactionManager")
public PlatformTransactionManager operatonTransactionManager(@Qualifier("operatonBpmDataSource") DataSource dataSource) {
...
This configuration adds support for `history-level auto` when the automatic database schema inspection is not available.
...
- `org.springframework.core.task.TaskExecutor` named `operatonTaskExecutor`
...
Applies the authorization configuration to the process engine. If not configured, the `operaton` default values are used (see [properties](#operaton-engine-properties)).
...
<td>
Registers a redirect from <code>/</code> to operaton's bundled <code>index.html</code>.
<br/>
...
<td><code>/operaton</code></td>
</tr>

<tr>
  <td rowspan="10"><a id="csrf"></a><code>operaton.bpm.webapp.csrf</code></td>
</tr>
<tr>
...
[operaton.bpm.webapp.session-cookie](../spring-boot-integration/configuration.mdx#session-cookie) in the `application.yaml`.
