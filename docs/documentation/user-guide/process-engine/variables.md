...
![Example img](./img/variables-4.png)Variables
...
![Example img](./img/variables-5.png)Variables
...
![Example img](./img/variables-6.png)Variables
...
<operaton:executionListener event="end">
        <operaton:script scriptFormat="groovy"><![CDATA[execution.setVariable("aVariable", "aValue","aSubProcess");]]></operaton:script>
...
![Example img](./img/variables-1.png)Variables
...
:::warning[String length restriction]
...
:::note[Using Custom Objects in Task Forms]
...
:::warning[Inconsistent Variable States]
...
Variables of [any type](#supported-variable-values) can be declared as transient using the `Variables` class and setting the parameter `isTransient` to true.
...
<operaton:inputOutput>
  <operaton:inputParameter name="x">foo</operaton:inputParameter>
  <operaton:inputParameter name="willBeNull"/>
  <operaton:outputParameter name="y">${x}</operaton:outputParameter>
  <operaton:outputParameter name="z">${willBeNull == null}</operaton:outputParameter>
</operaton:inputOutput>
```
...
<operaton:inputOutput>
  <operaton:inputParameter name="x">
    <operaton:list>
      ...
      <operaton:value>${1 + 1}</operaton:value>
      <operaton:list>
        <operaton:value>1</operaton:value>
        <operaton:value>2</operaton:value>
        <operaton:value>3</operaton:value>
      </operaton:list>
    </operaton:list>
  </operaton:inputParameter>
  <operaton:outputParameter name="y">
    <operaton:map>
      <operaton:entry key="foo">bar</operaton:entry>
      <operaton:entry key="map">
        <operaton:map>
          <operaton:entry key="hello">world</operaton:entry>
          <operaton:entry key="camunda">bpm</operaton:entry>
        </operaton:map>
      </operaton:entry>
    </operaton:map>
...
```xml
<serviceTask operaton:class="org.operaton.bpm.example.ComplexCalculation">
  <extensionElements>
    <operaton:inputOutput>
      <operaton:inputParameter name="userId">${id}</operaton:inputParameter>
      <operaton:inputParameter name="costSum">${sum}</operaton:inputParameter>
      <operaton:outputParameter name="forecast">${realisticForecast}</operaton:outputParameter>
    </operaton:inputOutput>
  </extensionElements>
</serviceTask>
```
...
<serviceTask operaton:class="org.operaton.bpm.example.ComplexCalculation">
  <extensionElements>
    <operaton:inputOutput>
      <operaton:inputParameter name="userId">${id}</operaton:inputParameter>
      <operaton:inputParameter name="costSum">
        ```${mapA[costs] + mapB[costs] + mapC[costs]}
      </operaton:inputParameter>
      <operaton:outputParameter name="avgForecast">
...
:::note[No output mapping for multi-instance constructs]
