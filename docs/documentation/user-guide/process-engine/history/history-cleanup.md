...
History cleanup can be triggered manually or scheduled on a regular basis. Only [operaton-admins](../user-guide/process-engine/authorization-service.md#the-operaton-admin-group">}}) have permissions to execute history cleanup manually.
...
* [Historic Instance Permissions](../user-guide/process-engine/authorization-service.md#historic-instance-permissions) are not cleaned up.
* [History Cleanup Jobs](../user-guide/process-engine/history/history-cleanup.md#historycleanupjobs-in-the-historic-job-log">}}) are not removed from the historic job log.
...
and cleanup must be configured explicitly. Please note that this only works for the [removal-time based history cleanup strategy](../user-guide/process-engine/history/history-cleanup.md#removal-time-strategy">}}).
