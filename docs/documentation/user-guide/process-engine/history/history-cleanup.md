...
History cleanup can be triggered manually or scheduled on a regular basis. Only [operaton-admins](../../process-engine/authorization-service.md#the-operaton-admin-group) have permissions to execute history cleanup manually.
...
![Example img](./../img/history-cleanup-process-hierarchy.png)History Cleanup
...
* [Historic Instance Permissions](../../process-engine/authorization-service.md#historic-instance-permissions) are not cleaned up.
* [History Cleanup Jobs](../../process-engine/history/history-cleanup.md#historycleanupjobs-in-the-historic-job-log) are not removed from the historic job log.

## Cleanup internals

History cleanup is implemented via jobs and performed by the [job executor](../../process-engine/the-job-executor.md). It therefore competes for execution resources with other jobs, e.g. triggering of BPMN timer events.
...
If cleanup fails, the job executor's [retry mechanism](../../process-engine/the-job-executor.md#failed-jobs) applies. Once the cleanup job has run out of retries, it is not executed again until one of the following actions is performed:
...
```xml
<process id="oneTaskProcess" name="The One Task Process" isExecutable="true" operaton:historyTimeToLive="5">
...
</process>
```
...
```xml
<process id="oneTaskProcess" name="The One Task Process" isExecutable="true" operaton:historyTimeToLive="P5D">
...
</process>
```
...
The `batchOperationsForHistoryCleanup` property can be configured in Spring based application or via custom [Process Engine Plugins](../../process-engine/process-engine-plugins.md). It defines history time to live for each specific historic batch operation.
...
and cleanup must be configured explicitly. Please note that this only works for the [removal-time based history cleanup strategy](../../process-engine/history/history-cleanup.md#removal-time-strategy).
