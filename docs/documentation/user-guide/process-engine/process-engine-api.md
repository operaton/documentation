...
All services are stateless. This means that you can easily run Operaton on multiple nodes in a cluster, each going to the same database, without having to worry about which machine actually executed previous calls. Any call to any service is idempotent regardless of where it is executed.
...
*   Changing which user a task is assigned to or which users are in some way involved with the task.
...
The **ManagementService** is typically not needed when coding custom applications. It allows you to retrieve information about the database tables and table metadata. Furthermore, it exposes query capabilities and management operations for jobs. Jobs are used in the engine for various things such as timers, asynchronous continuations, delayed suspension/activation, etc. Later on, these topics will be discussed in more detail.

The **FilterService** allows you to create and manage filters. Filters are stored queries like task queries. For example filters are used by Tasklist to [filter user tasks](../../webapps/tasklist/filters.md).

The **ExternalTaskService** provides access to [external task instances](external-tasks.md). External tasks represent work items that are processed externally and independently of the process engine.
...
The **[DecisionService](../process-engine/decisions/decision-service.md)** allows you to evaluate decisions that are deployed to the engine. It is an alternative to evaluate a decision within a business rule task that is independent from a process definition.

:::warning[Java Docs]
  For more detailed information on the service operations and the engine API, see the <a class="javadocref" href="">Java Docs</a>.
:::
...
The Java Query API allows you to write completely type-safe queries with a fluent API. You can add various conditions to your queries (all of which are applied together as a logical AND) and precisely one ordering. The following code shows an example:
...
[Delegation Code](../process-engine/delegation-code.md))
...
* Performing a query-based synchronous operation that affects more instances than the limit of
 maximum results (please use a [Batch Operation](../process-engine/batch-operations.md) instead)
...
:::note[Tutorial]
  [Performance Tuning with Custom Queries](http://blog.operaton.org/post/2017/12/custom-queries).
:::


### SQL Queries

The [table layout](database/database-schema/database-schema.md) is pretty straightforward - we focused on making it easy to understand. Hence it is OK to do SQL queries for e.g., reporting use cases. Just make sure that you do not mess up the engine data by updating the tables without exactly knowing what you are doing.
