...
creation time validation](#creation-time-validation) for details.
...
activity instances. See the [chapter on execution time validation](#execution-time-validation) for details.
...
events](#events) for details. When generating a migration
plan, it is possible to define this setting for generated instructions on
[User Tasks](#user-task) containing `timeout` task listeners and
...
[chapter on validation](#validation) to learn which kind of validation is performed before
...
For the associated timers, the considerations of [catching events](#events) apply here as well. On migration of the user task,
...
The considerations for [intermediate catch events](#events) apply here as well.

### External Task

When an active [external task](external-tasks.md) is migrated, all properties of the external task instance (i.e., `org.operaton.bpm.engine.externaltask.ExternalTask`) are preserved
...
:::warning[Limitation with asyncAfter]
...
* The overall assignment must be executable. See the [validation chapter](#validation) for details.
