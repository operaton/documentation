...
For [supported environments](../../introduction/supported-environments.md#container-managed-process-engine-and-operaton-cockpit-tasklist-admin), Operaton provides server modules that integrate job execution with the application server's managed thread pools. If you are using one of those environments, it is recommended to use the integration provided with it.
