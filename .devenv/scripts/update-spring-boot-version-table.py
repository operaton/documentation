#!/usr/bin/env python3
"""Update the Spring Boot version compatibility table in the documentation.

Fetches all final releases from https://github.com/operaton/operaton,
looks up the Spring Boot version from each release's root pom.xml
(property ``version.spring-boot``), and rewrites the compatibility table in:

    docs/documentation/user-guide/spring-boot-integration/version-compatibility.md
...
Rules applied:
  ...
  - Rows for new releases are added with the Spring Boot version read from
...
DOCS_FILE = "docs/documentation/user-guide/spring-boot-integration/version-compatibility.md"
...
def get_spring_boot_version(tag: str) -> Optional[str]:
    """Return the value of the ``version.spring-boot`` property in pom.xml at *tag*."""
    try:
        data = _github_get(f"/repos/{REPO}/contents/pom.xml?ref={tag}")
        content = base64.b64decode(data["content"]).decode()
        start = content.find("<version.spring-boot>")
        if start != -1:
            start += len("<version.spring-boot>")
            end = content.find("</version.spring-boot>", start)
            if end != -1:
                return content[start:end].strip()
        print(
            f"  Warning: <version.spring-boot> not found in pom.xml at {tag}",
            file=sys.stderr,
        )
...
def parse_table_rows(content: str) -> list[tuple[str, str]]:
    """Return list of (operaton_cell, spring_boot_cell) for every data row."""
...
def classify_existing_rows(
    ...
    for op_cell, sb_cell in existing_rows:
        m = _VERSION_RE.search(op_cell)
        if m:
            ver = (int(m.group(1)), int(m.group(2)), int(m.group(3)))
            if ver in release_set:
                kept.append((op_cell, sb_cell))
...
def fetch_new_rows(new_versions: set[tuple[int, int, int]]) -> list[tuple[str, str]]:
    """Fetch Spring Boot versions for new releases and build rows."""
    new_rows: list[tuple[str, str]] = []
    for major, minor, patch in sorted(new_versions):
        tag = f"v{major}.{minor}.{patch}"
        print(f"  Fetching : {tag}…")
        raw_sb = get_spring_boot_version(tag)
        if raw_sb:
            op_cell = f"{major}.{minor}.{patch}"
            print(f"             {op_cell} → Spring Boot {raw_sb}")
            new_rows.append((op_cell, raw_sb))
        else:
            print(f"  Skipping {tag}: could not determine Spring Boot version", file=sys.stderr)
    return new_rows
...
def main(dry_run: bool = False) -> None:
    ...
    new_rows = fetch_new_rows(release_set - listed_versions)
